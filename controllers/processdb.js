...
/**
 * Función que obtiene las estadisticas de uso de los procesos y las inserta en base de datos
 */
var processdb_linux = function () {
	var date = new Date();
	
	exec('sh ' + __dirname + '/bin/db_processes.sh', function(error, stdout, stderr) {
...
/**
 * Función que inserta un registro estadistico de los procesos en la base de datos 
 */
var insert = function (row) {
	typeof odm.insert(row,function (data) {
		if (data) {
			switch(data.state) {
				case INSERT_ERROR:
					console.log(row.date + ' [ERROR] [PROCESS] - ' + data.message);
				break;
				case INSERT_OK:
					console.log(row.date + ' [INFO] [PROCESS] - Se inserto un nuevo registro');
				break;
			}
		}
	});
