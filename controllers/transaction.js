 ...
 */
var transaction_linux = function () {
	var date = new Date();
	
	exec('sh ' + __dirname + '/bin/db_transactions.sh', function(error, stdout, stderr) {
...
var insert = function (row) {
	typeof odm.insert(row,function (data) {
		if (data) {
			switch(data.state) {
				case INSERT_ERROR:
					console.log(row.date + ' [ERROR] [TRANSACTION] - ' + data.message);
				break;
				case INSERT_OK:
					console.log(row.date + ' [INFO] [TRANSACTION] - Se inserto un nuevo registro');
				break;
			}
		}
	});
