...
/**
 * Función que obtiene las estadisticas de uso de los tablespace y las inserta en base de datos
 */
var database_linux = function () {
	var date = new Date();
	
	exec('sh ' + __dirname + '/bin/db_tablespaces.sh', function (error, stdout, stderr) {
		...
		for (var i = 0; i < array.length && array[i].length > 0; i++) {
			...
			if (tmp[0].indexOf('ORA-') == -1 && tmp[0].indexOf('ERROR:') == -1) {
				obj.database={
					'tablespace': tmp[0],
					'sizeMB': tmp[1],
					'freeMB': tmp[2],
...
/**
 * Función que inserta un registro estadistico de los tablespace en la base de datos 
 */
var insert = function (row) {
	typeof odm.insert(row,function (data) {
		if (data) {
			switch(data.state) {
				case INSERT_ERROR:
					console.log(row.date+' [ERROR] [DATABASE] - '+data.message);
				break;
				case INSERT_OK:
					console.log(row.date+' [INFO] [DATABASE] - Se inserto un nuevo registro');
				break;
			}
		}
	});
