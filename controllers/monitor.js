 ...
 */
var monitor_linux = function () {
	var date = new Date();
	
	exec('sh ' + __dirname + '/bin/db_monitor.sh', function (error, stdout, stderr) {
		...
		for (var i = 0; i < array.length && array[i].length > 0; i++) {
			...
			if (tmp[0].indexOf('ORA-') == -1 && tmp[0].indexOf('ERROR:') == -1 && tmp[0].indexOf('timeout') == -1 && tmp[0] != null) {
				obj.monitor = {
					'response': tmp[0],
					'time':     tmp[1]
				}
...
var insert = function (row) {
	typeof odm.insert(row,function (data) {
		if (data) {
			switch(data.state) {
				case INSERT_ERROR:
					console.log(row.date + ' [ERROR] [MONITOR] - ' + data.message);
				break;
				case INSERT_OK:
					console.log(row.date + ' [INFO] [MONITOR] - Se inserto un nuevo registro');
				break;
			}
		}
	});
